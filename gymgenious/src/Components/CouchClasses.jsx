    ...
    try {
      ...
      const dataMatches = calendarEvents.map(evento => {
        const comment = assistance_references.find(c => 
          ...
          .toLocaleDateString('es-CA', argentinaDateOptions) // es-CA para obtener formato YYYY-MM-DD
          .replace(/\//g, '-') === new Date(c.date)
          .toLocaleDateString('es-CA', argentinaDateOptions)
          .replace(/\//g, '-'))
        );
        return {
          ...evento,
      ...
      console.log("asi se ven las clases", new Date(calendarEvents[7].start)
      .toLocaleDateString('es-CA', argentinaDateOptions)
      .replace(/\//g, '-'));
      console.log(calendarEvents[7])
